...
namespace ThoiTrangNam.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            foreach(var orderDetail in orderDetails)
            {
                if (orderDetail.Rating != null)
                {
                    ...
                    reviewViewModel.Rating = orderDetail.Rating ?? 0;
        ...
        {
            if (ModelState.IsValid && review.Rating != 0)
            {
                await _orderDetailRepository.UpdateReview(review.OrderDetailId, review.Rating, review.UserReview);
                var detail = await _orderDetailRepository.GetByIdAsync(review.OrderDetailId);
                await _productRepository.UpdateRating(detail.ProductId, review.Rating);
